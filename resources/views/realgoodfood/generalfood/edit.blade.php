    		...
    		<div class="col-md-12">
    			<div class="portlet box green-haze ">
    				<div class="portlet-title">
    					<div class="caption">
    						<i class="fa fa-home"></i> Update General Food
    					</div>
    				</div>
    				<div class="portlet-body form">
    					@include('common.alert')
    					<form class="form-horizontal" action="{{ url('general-food/'.$food['id']) }}" method="post" role="form" enctype="multipart/form-data">
    						{{ csrf_field() }}
    						{{method_field('PUT')}}
    						<div class="form-body">
    							<div class="form-group" >
    								...
    								<div class="col-md-9 {{ $errors->has('restaurantid') ? 'has-error' : '' }}">
    									<select class="form-control select2" name="restaurantid" id="restaurantid">
    										...
    										<option value="{{ $restaurantid }}" {{ isset($food) ? ($food['restaurantid']==$restaurantid ? 'selected':'')  : (old('restaurantid') == $restaurantid ? 'selected' : '') }}>
    											{{ $restaurantname }}
    										</option>
    										@endforeach
    										@endif
    							...
    							<div class="form-group" >
    								...
    								<div class="col-md-9 {{ $errors->has('categoryid') ? 'has-error' : '' }}">
    									<select class="form-control select2" name="categoryid" id="categoryid">
    										...
    										<option value="{{ $gnl_categoryid }}" {{ isset($food) ? ($food['categoryid']==$gnl_categoryid ? 'selected':'')  : (old('categoryid') == $categoryid ? 'selected' : '') }}>
    											{{ $gnl_categorname }}
    										</option>
    										@endforeach
    										@endif
    							...
    							<div class="form-group">
    								...
    								<div class="col-md-9 {{ $errors->has('foodname') ? 'has-error' : '' }}">
    									<input type="text" class="form-control" name="foodname" value="{{ isset($food)?$food['foodname'] : '' }}" placeholder="Enter  food name">
    								</div>
    							</div>
    							...
    							<div class="form-group">
    								...
    								<div class="col-md-9 {{ $errors->has('foodnamecolor') ? 'has-error' : '' }}">
    									<input type="text" class="form-control" name="foodnamecolor" value="{{ isset($food)?$food['foodnamecolor'] : '' }}" placeholder="Enter food name color">
    								</div>
    							</div>
    							...
    							<div class="form-group">
    								...
    								<div class="col-md-9 {{ $errors->has('otherdetail') ? 'has-error' : '' }}">
    									<textarea class="form-control" name="otherdetail" rows="3" cols="4" placeholder="Enter details" >{{ isset($food)?$food['otherdetail'] : '' }}</textarea>
    								</div>
    							</div>
    								...
    								<div class="col-md-9">
    									<div class="fileinput fileinput-new" data-provides="fileinput">
    										<div class="fileinput-new thumbnail" >
    											@if(!empty($food['originalpicture']))
    											<img src="{{ url('upload/menu/thumbnail_images/'.$food['originalpicture']) }}" alt="preview" id="preview-image" />
    											@else
    								...
    								<div class="col-md-9">
    									<div class="radio-list">
    										<label class="radio-inline">
    											<input type="radio" name="status" value="1" {{ isset($food) ? ($food['status'] !=0 ? 'checked':'')  : (old('status') !=0 ? 'checked' : '') }}  > Published </label>
    											<label class="radio-inline">
    												<input type="radio" name="status" value="0" {{ isset($food) ? ($food['status'] ==0 ? 'checked':'')  : (old('status') ==0 ? 'checked' : '') }}  > Not Published </label>
    												<label class="radio-inline">
    												</div>
    											</div>
    										</div>
